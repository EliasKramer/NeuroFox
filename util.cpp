...
float random_float_excl(float min, float max)
{
	...
	if (ret_val < min || ret_val >= max)
	{
		...
		std::cout
			<< "random_float_excl returned a value outside of the range "
			<< min << " to " << max << std::endl
			<< "returned value: " << ret_val
			<< std::endl;
		return min;
	}
...
std::string ms_to_str(size_t ms)
{
	...
	size_t week_count = ms / w;
	size_t day_count = (ms - week_count * w) / d;
	size_t hour_count = (ms - week_count * w - day_count * d) / h;
	size_t minute_count = (ms - week_count * w - day_count * d - hour_count * h) / min;
	size_t second_count = (ms - week_count * w - day_count * d - hour_count * h - minute_count * min) / s;
	size_t millisecond_count = ms - week_count * w - day_count * d - hour_count * h - minute_count * min - second_count * s;
	...
	if (millisecond_count > 0)
		result += std::to_string(millisecond_count) + "ms ";
